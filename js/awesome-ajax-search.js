...
************************/
		...
		pluginOptions : function(){
			...
			self.alsohide = ( self.options.alsohide ) ? self.options.alsohide : "#content article.hentry, #comments, #content header";
		...
		doingTime : function(trigger){
			...
			self.interval = setInterval(function(){
				count++;
				if ( count >= 1 ){ //these numbers matter
					self.loadSearch(trigger);
					clearInterval(self.interval);
				}
			}, self.intervalLength); //these numbers matter
		},
